...
class Build():
    ...
    def _BuildPlatform(self):
        for BuildTarget in self.BuildTargetList:
            for ToolChain in self.ToolChainList:
#                print 'reday to get a object WorkspaceAutoGen'
                Wa = WorkspaceAutoGen(
                        self.WorkspaceDir,
                        self.Platform,
                        BuildTarget,
    ...
    def Launch(self):
        if self.ModuleFile == None or self.ModuleFile == "":
            if not self.SpawnMode or self.Target not in ["", "all"]:
                ...
                #print 'comming build platform'
                self._BuildPlatform()
                print 'comming out build platfomr'
...
def MyOptionParser():
    ...
    Parser.add_option("-a", "--arch", action="append", type="choice", choices=['IA32','X64','IPF','EBC','ARM', 'MIPS64'], dest="TargetArch",
        help="ARCHS is one of list: IA32, X64, IPF, ARM MIPS64 or EBC, which overrides target.txt's TARGET_ARCH definition. To specify more archs, please repeat this option.")
